...
static void event_thread(int event_fd) {
    ...
    while (g_running) {
        ...
        if (msg.method_id == EVENT_STATUS && msg.payload.size() >= 8) {
            ...
            uint32_t speed = someip::legacy::load_u32(msg.payload.data());
...
static std::pair<uint8_t, std::vector<uint8_t>> request(
    ...
    req.to_buf(data);
    someip::legacy::send_to(method_fd, endpoint, data.data(), data.size());
...
int main() {
    ...
    int method_fd = someip::legacy::make_udp_socket(0, false);
    int event_fd = someip::legacy::make_udp_socket(0, true);
    int sd_fd = someip::legacy::make_sd_socket(interface_ip);
    ...
    // ---- SubscribeEventgroup over SD multicast ----
    {
        ...
        someip::legacy::build_entry(someip::legacy::EntryType::SUBSCRIBE_EVENTGROUP, SERVICE_ID,
                        INSTANCE_ID, MAJOR_VERSION, someip::legacy::SD_TTL, 0,
                        EVENTGROUP_MAIN, 0, 1, entry);
        entries.push_back(entry);
        std::vector<uint8_t> option;
        someip::legacy::build_ipv4_endpoint_option(interface_ip, event_port, option);
        options.push_back(option);
        std::vector<uint8_t> data;
        uint16_t session = next_session();
        someip::legacy::build_sd_message(entries, options, session, data);
        ...
        while (!acked && std::chrono::steady_clock::now() < deadline) {
            ...
            for (uint32_t i = 0; i + 16 <= entries_len; i += 16) {
                ...
                if (e[0] != someip::legacy::EntryType::SUBSCRIBE_EVENTGROUP_ACK) continue;
                if (someip::legacy::load_u16(e + 4) != SERVICE_ID) continue;
                uint16_t eg = someip::legacy::load_u16(e + 12);
    ...
    std::vector<std::pair<std::string, uint16_t>> offers;
    {
        ...
        entries.push_back([&]() {
            ...
            someip::legacy::build_entry(someip::legacy::EntryType::FIND_SERVICE, SERVICE_ID, INSTANCE_ID,
    ...
    while (offers.empty()) {
        ...
        while (oc + 4 <= options_len) {
            ...
            if (opts[oc + 2] == someip::legacy::OptionType::IPV4_ENDPOINT && olen >= 9) {
                ...
                for (uint32_t i = 0; i + 16 <= entries_len; i += 16) {
                    const uint8_t *e = entries_ptr + i;
                    if (e[0] == someip::legacy::EntryType::OFFER_SERVICE &&
                        someip::legacy::load_u16(e + 4) == SERVICE_ID &&
                        someip::legacy::load_u16(e + 6) == INSTANCE_ID) {
    ...
    sockaddr_in endpoint = someip::legacy::make_addr(offers[0].first, offers[0].second);
    ...
    if (rc0.first != 0xFF && rc0.second.size() >= 10) {
        uint32_t sid = someip::legacy::load_u32(rc0.second.data());
        uint32_t iid = someip::legacy::load_u32(rc0.second.data() + 4);
    ...
    someip::legacy::push_u32(add_payload, 3);
    someip::legacy::push_u32(add_payload, 4);
    ...
    if (rc1.first == 0 && rc1.second.size() >= 4)
        printf(" result=%u", someip::legacy::load_u32(rc1.second.data()));
    printf("\n");
    ...
    if (rc2.first == 0 && rc2.second.size() >= 4)
        printf(" value=%u km/h", someip::legacy::load_u32(rc2.second.data()));
    printf("\n");

    std::vector<uint8_t> speed_payload;
    someip::legacy::push_u32(speed_payload, 88);
    ...
    if (rc4.first == 0 && rc4.second.size() >= 4)
        printf(" value=%u km/h", someip::legacy::load_u32(rc4.second.data()));
    printf("\n");
