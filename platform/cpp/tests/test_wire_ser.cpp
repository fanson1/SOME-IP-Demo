...
#include "someip/wire.hpp"
#include "someip/ser.hpp"
...
#define CHECK_THROWS(expr)                                                \
    do {                                                                  \
        bool threw = false;                                               \
        try { (void)(expr); }                                             \
        catch (const someip::MalformedMessage &) { threw = true; }       \
        CHECK(threw);                                                     \
    } while (0)
...
static void test_wire_roundtrip() {
    ...
    someip::Message parsed = someip::Message::from_bytes(GOLDEN, sizeof(GOLDEN));
...
static void test_wire_validation() {
    CHECK_THROWS(someip::Message::from_bytes(GOLDEN, 8));      // short
    CHECK_THROWS(someip::Message::decode_header(GOLDEN, 4));    // very short

    std::vector<uint8_t> truncated(GOLDEN, GOLDEN + 20);        // partial payload
    CHECK_THROWS(someip::Message::from_bytes(truncated.data(), truncated.size()));

    // length field < 8 must be rejected.
    std::vector<uint8_t> bad(GOLDEN, GOLDEN + sizeof(GOLDEN));
    bad[4] = 0; bad[5] = 0; bad[6] = 0; bad[7] = 4;
    CHECK_THROWS(someip::Message::from_bytes(bad.data(), bad.size()));
}
...
static void test_ser_structure() {
    someip::Writer w;
    w.struct_([&](someip::Writer &inner) {
        inner.u8(7).string("tag");
    });
    someip::Reader rw(w.data());
    rw.struct_([&](someip::Reader &inner) {
        CHECK(inner.u8() == 7);
    ...
    someip::Writer short_w;
    short_w.u16(1);
    someip::Reader short_r(short_w.data());
    short_r.u16();
...
static void test_v1_interop() {
    ...
    someip::Message m = someip::Message::from_bytes(v1.data(), v1.size());
